...
func Queue[V abs.ValueLike]() abs.QueueLike[V] {
	return QueueWithCapacity[V](0)
}

// This constructor creates a new empty queue with the specified capacity.
func QueueWithCapacity[V abs.ValueLike](capacity int) abs.QueueLike[V] {
	// Groom the arguments.
	if capacity < 1 {
...
// AsArray() method.
type queue[V abs.ValueLike] struct {
	available chan bool
...
func (v *queue[V]) AddValues(values abs.Sequential[V]) {
...
// value type.
func Queues[V abs.ValueLike]() *queues[V] {
	return &queues[V]{}
}
...
//   - V is any type of value.
type queues[V abs.ValueLike] struct{}
...
func (l *queues[V]) Fork(wg *syn.WaitGroup, input abs.FIFO[V], size int) abs.Sequential[abs.FIFO[V]] {
	...
	var outputs = List[abs.FIFO[V]]()
	for i := 0; i < size; i++ {
		outputs.AddValue(abs.FIFO[V](QueueWithCapacity[V](capacity)))
	}
	...
	go func() {
		...
		var iterator = age.Iterator[abs.FIFO[V]](outputs)
		for {
...
// the Join() function.
func (l *queues[V]) Split(wg *syn.WaitGroup, input abs.FIFO[V], size int) abs.Sequential[abs.FIFO[V]] {
	// Validate the arguments.
	if size < 1 {
...
func (l *queues[V]) Join(wg *syn.WaitGroup, inputs abs.Sequential[abs.FIFO[V]]) abs.FIFO[V] {
	// Validate the arguments.
